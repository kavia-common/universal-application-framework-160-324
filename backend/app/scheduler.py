...
class IdleShutdownScheduler:
    """
    A lightweight background thread scheduler that periodically:
      - Lists running instances
      - Determines idle instances (based on configured threshold)
      - Stops them and logs events in DB
    """
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=self._run, name="IdleShutdownScheduler", daemon=True)
        self._thread.start()
        logger.info("IdleShutdownScheduler started.")
    ...
    def stop(self):
        ...
        logger.info("IdleShutdownScheduler stopped.")

    def _run(self):
        boto_session = get_boto3_session(self.settings)
        while not self._stop_event.is_set():
            try:
                with db_session() as db:  # type: Session
                    self._tick(db, boto_session)
            except Exception as e:
                logger.exception("Scheduler tick failed: %s", e)
            self._stop_event.wait(self.settings.scheduler_interval_seconds)

    def _tick(self, db: Session, boto_session):
        try:
            running = list_running_instances(self.settings, boto_session)
            idle = determine_idle_instances(running, self.settings.idle_threshold_minutes)
            if idle:
                results = stop_instances_and_log(self.settings, boto_session, db, idle)
                for inst, ok, msg in results:
                    level = logging.INFO if ok else logging.WARNING
                    logger.log(level, "Stop result for %s: %s", inst.instance_id, msg)
            else:
                logger.debug("No idle instances found this cycle.")
        except Exception as e:
            logger.exception("Error in scheduler _tick: %s", e)
```

Explanation: Create FastAPI app with dependency injection, OpenAPI docs, routes for listing instances, manual trigger, and startup/shutdown hooks.
````write file="universal-application-framework-160-324/backend/app/main.py"
from __future__ import annotations
...
app = FastAPI(
    ...
    description=(
        ...
        "and log shutdown events into PostgreSQL."
    ),
    version="1.0.0",
    openapi_tags=[
                ...
                if ok:
                    events.append(
                        ShutdownEvent(
                            ...
                            event_time=None,  # Will be replaced by DB value in most cases, but returning now-time
                        )
                    )
